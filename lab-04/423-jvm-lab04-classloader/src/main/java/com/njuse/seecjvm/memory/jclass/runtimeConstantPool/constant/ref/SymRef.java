...
public abstract class SymRef implements Constant {
    ...
    public void resolveClassRef() throws ClassNotFoundException, IllegalAccessException {
        ...
        JClass currentClass = this.runtimeConstantPool.getClazz();

        JClass targetClass = ClassLoader.getInstance().loadClass(this.className, currentClass.getLoadEntryType());

        if (!targetClass.isAccessibleTo(currentClass)) {
            throw new IllegalAccessException(this.className + "is not accessible to" + currentClass.getName());
        }
