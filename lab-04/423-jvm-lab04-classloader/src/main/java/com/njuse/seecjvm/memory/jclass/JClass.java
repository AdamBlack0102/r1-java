...
public class JClass {
    ...
    public boolean isAccessibleTo(JClass caller) {
        ...
        String callerPackageName = caller.getPackageName();

        if (thisPackageName.equals(callerPackageName)) {
            return true;
        }

        if ((this.accessFlags & AccessFlags.ACC_PROTECTED) != 0) {
            JClass superClass = caller.getSuperClass();
